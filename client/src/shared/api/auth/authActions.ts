...
import { LoginData, UserAttributes, UserUpdateAttributes } from '@/features/auth/model/model';

export const register = createAsyncThunk('auth/register', async (data: UserAttributes) => {
...
export const loadUser = createAsyncThunk('auth/loadUser', async (userId: string) => {
  const token = localStorage.getItem('token');
