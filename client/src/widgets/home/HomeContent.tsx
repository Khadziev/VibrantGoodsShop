import React, { lazy, Suspense } from 'react';
import { useFetchAllProductsQuery } from '@/shared/api/userApi';
import { Loading, Button } from '@/shared/ui';
import { Link } from 'react-router-dom';
import Slider from '@/widgets/carousel/Slider';
import Sidebars from '@/widgets/Poster/Sidebar/Sidebars';
import Poster from '@/widgets/Poster/Banner/Poster';
import Banner from '@/widgets/banners';
import BrandList from '@/widgets/Brands/BrandList';
import ProductItemHome from '@/entities/product/ui/ProductItemHome/ProductItemHome';

const DiscountedItems = lazy(() => import('@/pages/discount/Discounted/DiscountedItems'));
const Banners = lazy(() => import('@/widgets/Poster/Banner/Banners'));
const Category = lazy(() => import('@/entities/category/ui/category/Category'));

const HomeContent: React.FC = () => {
  const { data, isLoading } = useFetchAllProductsQuery(null);

  if (isLoading || !data) {
    return <Loading />;
  }

  return (
    <div className="pt-15">
      <div className="pt-12">
        <Slider />
      </div>
      <div className="flex pt-4">
        <Sidebars />
        <Poster />
      </div>
      <Suspense fallback={<Loading />}>
        <div>
          <DiscountedItems />
        </div>
        <div>
          <Banners />
        </div>
        <div>
          <div className="flex">
            {data.slice(0, 4).map((item) => (
              <div key={item._id}>
                <ProductItemHome item={item} />
              </div>
            ))}
          </div>
        </div>
        <div className="flex justify-center mt-5 w-full">
          <Link to="/data/all">
            <Button variant="primary" size="md">
              Посмотреть все
            </Button>
          </Link>
        </div>
        <div>
          <Category />
        </div>
        <div>
          <Banner />
        </div>
      </Suspense>
      <div>
        <BrandList />
      </div>
    </div>
  );
};

export default HomeContent;
